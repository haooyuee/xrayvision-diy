...
def train(model, dataset, cfg):
    ...
    auc_test = None
    metrics = []
    weights_files = glob(join(cfg.output_dir, f'{dataset_name}-e*.pt'))  # Find all weights files###e* to 89
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        if np.mean(auc_valid) > best_metric:
            ...
            weights_for_best_validauc = model.state_dict()
            torch.save(model, join(cfg.output_dir, f'{dataset_name}-e{epoch}-best-.pt'))
