...
def train_epoch(cfg, epoch, model, device, train_loader, valid_loader, optimizer, criterion, limit=None):
    model.train()
    weights = None
    if cfg.taskweights:
    ...
    for batch_idx, samples in enumerate(t):
        ...
        if batch_idx % 100 == 0:
            ...
            with torch.no_grad():    
                ...
                for jdx, samples in valid_loader:
                    images = samples["img"].float().to(device)
                    targets = samples["lab"].to(device)
            ...
            print ('Epoch=%s, BatchID=%s, Val_AUC=%.4f, lr=%.4f'%(epoch, batch_idx, val_auc,  optimizer.lr))

    return np.mean(avg_loss)

def valid_test_epoch(name, epoch, model, device, data_loader, limit=None): #remove criterion
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, samples in enumerate(t):
            ...
            for task in range(targets.shape[1]):
                ...
                task_target = task_target[mask]
                if len(task_target) > 0:
                    loss += torch.nn.BCEWithLogitsLoss(task_output.double(), task_target.double())
                    #print('+')
                    #print(loss)
